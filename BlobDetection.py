...
while True:
	...
	cv2.imshow("Arrange Tracker", blurred_bgr)
...
#show blurred_bgr images
cv2.imshow("BGR image", blurred_bgr)
cv2.waitKey(0)
...
# diff = [i * 0.15 for i in selected_color]
# print("diff = " + str(diff))
...
#hard coded filter
lower_bound = (0, 220, 158)
upper_bound = (12, 255, 255)
...
# result = cv2.bitwise_and(blurred_bgr, blurred_bgr, mask = mask) 
...
im_with_keypoints = cv2.drawKeypoints(blurred_bgr, keypoints, np.array([]), (0,0,255), cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
...
############################## use knowledge to blob detect over video #############################
while True:
	# grab image
	ret, bgr = camera.read()
	...
	# creates a mask with the predefined lower and upper bounds
	mask = cv2.inRange(hsv, lower_bound, upper_bound)

	#invert the mask to blob detect
	inverted = cv2.bitwise_not(mask)
